...
- Pods calculating Fibonacci number for 25:
  - testpod-6794d4cfdc-2gs9b
  - testpod-6794d4cfdc-6r6lg
  - testpod-6794d4cfdc-rvmd2

- Pods calculating Fibonacci number for 26:
  - testpod-6794d4cfdc-jgxqg
  - testpod-6794d4cfdc-stjzd

- Pods calculating Fibonacci number for 27:
  - testpod-6794d4cfdc-7rrwr
  - testpod-6794d4cfdc-gv856
  - testpod-6794d4cfdc-jz462
  - testpod-6794d4cfdc-kr64w
  - testpod-6794d4cfdc-kxhwx
  - testpod-6794d4cfdc-m2xcx
  - testpod-6794d4cfdc-p594m
  - testpod-6794d4cfdc-qnlnl
  - testpod-6794d4cfdc-tffd9

- Pod calculating Fibonacci number for 29:
  - testpod-6794d4cfdc-mp8sn
...
![Total CPU usage before optimization](images/total-cpu-usage-before.png)
    ...
    sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_rate{container!="POD", container=~"testpod"})
    <br><br></i></figcaption>
...
![alt text](images/per-pod-cpu-before.png)
    ...
    sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_rate{container!="POD", container="testpod"}) by (pod)<br><br></i></figcaption> 
- Service Response Time:
![alt text](images/latencies-before.png)
    <figcaption><i>Fig 4: Service latencies before optimization:<ul>
    <li>histogram_quantile(0.50, sum(rate(istio_request_duration_milliseconds_bucket{reporter="destination",destination_canonical_service="testpod"}[2m])) by (le,destination_canonical_service))</li>
    <li>histogram_quantile(0.90, sum(rate(istio_request_duration_milliseconds_bucket{reporter="destination",destination_canonical_service="testpod"}[2m])) by (le,destination_canonical_service))</li>
    <li>histogram_quantile(0.95, sum(rate(istio_request_duration_milliseconds_bucket{reporter="destination",destination_canonical_service="testpod"}[2m])) by (le,destination_canonical_service))</li>
    <li>histogram_quantile(0.99, sum(rate(istio_request_duration_milliseconds_bucket{reporter="destination",destination_canonical_service="testpod"}[2m])) by (le,destination_canonical_service))</li>
    </ul></i></figcaption>
  ...
  ![alt text](images/per-pod-p50-before.png)
    ...
    histogram_quantile(0.5, sum(rate(istio_request_duration_milliseconds_bucket{reporter="destination",destination_canonical_service="testpod",request_protocol="http",response_code=~"2.*",pod=~"testpod.*"}[2m])) by (le,pod))<br><br></i></figcaption>
  ...
  ![alt text](images/per-pod-p90-before.png)
    ...
    histogram_quantile(0.9, sum(rate(istio_request_duration_milliseconds_bucket{reporter="destination",destination_canonical_service="testpod",request_protocol="http",response_code=~"2.*",pod=~"testpod.*"}[2m])) by (le,pod))<br><br></i></figcaption>  
  ...
  ![alt text](images/per-pod-p95-before.png)
    ...
    histogram_quantile(0.95, sum(rate(istio_request_duration_milliseconds_bucket{reporter="destination",destination_canonical_service="testpod",request_protocol="http",response_code=~"2.*",pod=~"testpod.*"}[2m])) by (le,pod))<br><br></i></figcaption>    
  ...
  ![alt text](images/per-pod-p99-before.png)
    ...
    histogram_quantile(0.99, sum(rate(istio_request_duration_milliseconds_bucket{reporter="destination",destination_canonical_service="testpod",request_protocol="http",response_code=~"2.*",pod=~"testpod.*"}[2m])) by (le,pod))<br><br></i></figcaption>
...
![alt text](images/per-pod-rps-before.png)
    ...
    sum(rate(istio_requests_total{container!="POD",destination_canonical_service=~"testpod",pod=~"testpod.*"})) by (pod)<br><br></i></figcaption>
### After Optimization
...
![alt text](images/total-cpu-usage.png)
    ...
    sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_rate{container!="POD", container=~"testpod"})<br><br>
    </i></figcaption>
